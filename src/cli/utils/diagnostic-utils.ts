 ...
 */
export function severityToString(severity: number | undefined): 'error' | 'warning' | 'info' | 'hint' {
    switch (severity) {
 ...
 */
export function severityToSarifLevel(severity: number | undefined): 'none' | 'note' | 'warning' | 'error' {
    switch (severity) {
        case DiagnosticSeverity.Error:
            return 'error';
        case DiagnosticSeverity.Warning:
