...
import { validateFile, type ValidationResult } from '../../utils/parser-utils.js';
 ...
 */
export async function validateCommand(filePatterns: string[], options: ValidateOptions): Promise<void> {
 ...
 * Filter diagnostics based on options.
 */
function filterDiagnostics(diagnostics: Diagnostic[], options: ValidateOptions): Diagnostic[] {
    return diagnostics.filter(d => {
        // Always show errors
        if (d.severity === 1) return true;
        // Show warnings if requested
        if (d.severity === 2 && options.warnings) return true;
        // Show hints if requested
        if ((d.severity === 3 || d.severity === 4) && options.hints) return true;
        return false;
    });
}
 ...
 */
function formatResults(fileResults: FileResult[], options: ValidateOptions): string {
